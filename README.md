...
Canvas Pilot is an open-source, local-first Canvas LMS AI agent for Codex and
Claude Code power users. It learns each course's recurring assignment pattern,
then reuses that pattern through scan -> approval -> execute workflows that
produce review-ready drafts and run reports.

It is a Canvas MCP workflow layer, not just another Canvas API wrapper. Canvas
MCP servers let an agent access Canvas; Canvas Pilot adds recurring course
memory, assignment workflow automation, approval boundaries, verification, and
student review above the access layer.

The public repo contains the generic framework. Credentials, cookies, private
course identifiers, assignment inputs, drafts, and real run outputs stay local
and gitignored.

Codex is the sole primary driver. The legacy Claude Code path is preserved as
frozen history, but current setup, scan, drafting, submission, quiz, and cron
...
The default behavior is *produce verified drafts, you decide what to submit*.
Plan approval never grants Canvas write access. Interactive submission is a
separate, exact-target action with a verification gate and Canvas read-back.
...
1. **Opportunity analysis is read-only and evidence-based.** It reads
   enough representative assignment instructions to understand the real task,
   plus sanitized facts about retry and feedback policy. It never starts an
   attempt, solves work, retains raw answers or exact grades, uploads, submits,
   or creates a route. The agent makes a qualitative recommendation and stops
   for your numbered choice.

   Strong first-skill defaults include Canvas-submittable code, mostly
   objective quizzes, quantitative accounting/economics/statistics/math/finance,
   structured Word/Excel/PDF business work, and independent short answers or
   annotations. A main continuous prose unit around 200 words or more is a
   strong demoter; total word count is not the rule. A quiz with multiple
   attempts, feedback visible before retry, and keep-highest scoring can move
   forward because it has a useful post-submit correction loop.

2. **`canvas-bootstrap` is single-cluster on purpose.** A typical student has
...
3. **The first time you run a per-course skill, it is a co-authoring
...
1. Copy the concise prompt from <https://canvas-pilot.likelyou.com/install>
   into a local agent with shell and file access.
2. Let the agent download or update the repo. After download, it asks which
   school you use Canvas through and resolves that school's official Canvas
   login URL. It asks for the Canvas URL only when the school answer is
   ambiguous; it never guesses.
3. Complete login only in the browser it opens. As soon as authentication is
   verified, the agent runs `canvas-skill-opportunity` and stops for your
   numbered choice before route creation, homework scan, execution, or submission.

The prompt dispatches `canvas-setup`:

- Asks which school you use Canvas through, then resolves its official Canvas
  login URL. If that cannot be resolved unambiguously, it asks you for the URL
  instead of guessing.
- Installs Playwright + Chromium silently (~150 MB, one-time).
- Opens a browser. You log in to Canvas with your school SSO once.
  The browser session stays local. Username/password capture and persistence
  are off by default; setup removes any legacy record, then verifies that the
  retained session still authenticates.
- Verifies Canvas authentication without exposing credentials.
...
scan canvas
   ...
   -> optionally send one later `submit N` / `take quiz N` / `retake quiz N`
```
...
Canvas Pilot ships Codex skills under `.agents/skills/`. The framework skills
...
at runtime, without an overlay. Legacy Claude skill mirrors remain frozen under
`.claude/skills/` as historical reference only.

### Framework skills

| Skill | What it does |
|---|---|
| `canvas-setup` | First asks which school the user accesses Canvas through, resolves the official Canvas login URL (asking for the URL only if ambiguous), then handles local configuration, protected credentials, browser login, and auth verification. With empty routes, it immediately hands off to opportunity analysis and stops before homework scanning. |
| `canvas-skill-opportunity` | Uses read-only recurring evidence, representative real specs, and sanitized retry/feedback-policy facts to make qualitative first-skill recommendations. Writes a private report and stops for a numbered choice. |
| `canvas-bootstrap` | After the user selects a candidate, verifies its real spec and workflow, then drafts one public-safe per-course skill and local route. Includes a viability gate and first-run calibration loop. |
| `canvas-scan` | Reads `courses.yaml`, queries Canvas for assignments in the pending window, writes `plan.json`, and stops. Empty routes hand off to `canvas-skill-opportunity`, not directly to bootstrap. |
| `canvas-execute` | Reads the user's approval reply against `plan.json`, archives approved items' old result slots, dispatches approved items to their per-course skill, gathers results, writes `REPORT.md`, and mirrors drafts to `final_drafts/`. It does not infer submission authority from plan approval. |
| `canvas-submit` | Accepts one later exact `submit N`, `take quiz N`, or `retake quiz N`, revalidates the current plan and local evidence, issues one signed receipt, mutates only that target, and verifies Canvas by read-back. |
| `canvas-cron` | Manages disabled-first scan, email, and explicitly authorized autonomous templates with side-effect-free dry runs and post-state checks. |
...
reference-fetch regex patterns, verification rules, permitted submission
formats, and optional cron policy. Runtime mutation still requires the matching
signed receipt.
...
- Judge recurring assignment patterns from representative real specs and safe
  Canvas policy evidence so you can choose which durable course skill is worth
  building first.
...
- Pause and wait for your approval before any assignment work happens. The
  approval is a filesystem boundary, not a prose instruction.
...
- Verify drafts against numeric constraints in the spec (sentence counts, page
  counts, function counts) before any separately authorized mutation.
...
- Read or write content on your behalf without your approval at the plan-review
  step.
- Treat plan approval, an environment flag, or an old message as Canvas write
  authority. Interactive mutations require a current signed receipt; durable
  authority exists only in an explicitly installed autonomous cron workflow.
...
|   |-- canvas-setup/           # first-run config + auth
|   |-- canvas-skill-opportunity/ # read-only first-skill recommendation
|   |-- canvas-bootstrap/       # per-course overlay drafting
|   |-- canvas-scan/            # scan -> plan.json
|   |-- canvas-execute/         # dispatch approved items
|   |-- canvas-submit/          # one later exact Canvas mutation
|   |-- canvas-cron/            # disabled-first schedule management
...
### Drafts by default, submission is separately authorized

Default behavior: produce a verified draft, you upload manually. An interactive
Canvas write requires a later exact command and a short-lived receipt bound to
one origin, session, target, and action set. Durable authority is limited to an
explicitly installed cron parent receipt that derives short-lived per-item
children. Auto-submit is never the silent default.
...
- Do NOT leave the `.scan_in_progress` marker behind. If a run crashes,
  resume through the shared preparation/finalize path; never manually discard
  another session's marker. The Stop hook refuses release until ownership,
  prepared result slots, and every assignment result validate.
...
- **A driver-free standalone app.** Canvas Pilot is currently a local agent
  workflow. Codex is the sole primary driver, and the legacy Claude Code path
  is frozen history. A fully driver-free CLI or desktop app is not the current
  focus.
...
- Framework skills: `.agents/skills/canvas-{setup,skill-opportunity,bootstrap,scan,execute,skip}/SKILL.md`.
...
**Not sure which course workflow is worth building next?** Invoke
`canvas-skill-opportunity`. It refreshes the private recommendation and stops
