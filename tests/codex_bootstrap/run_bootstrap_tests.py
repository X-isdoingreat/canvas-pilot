...
def test_opportunity_handoff_and_viability_gate() -> None:
    text = read_skill()
    assert_pattern(text, r"canvas-skill-opportunity", "opportunity handoff")
    assert_pattern(text, r"skill-opportunities\.json", "local opportunity report")
    assert_pattern(text, r"viability gate", "viability gate")
    assert_pattern(text, r"real instructions|real spec", "real specification check")
    assert_pattern(text, r"measurable|verification\.log", "measurable verification")
    assert_pattern(text, r"do not\s+(?:write a ready route|mark.*route ready)", "no unverified ready route")
    assert_pattern(text, r"grade promise", "no grade promise")
...
def main() -> int:
    tests = [
        ...
        test_opportunity_handoff_and_viability_gate,
