...
Two kinds (CEO requirement: structural separation):
  ...
  AutonomousTemplate  — exec up to 30 min. Spawns fresh claude.exe with
                        spec.default_params["env_var_name"]=1 set in env.
...
then aborts the fire cleanly without launching Chromium. In dry_run=True
...
class ActionTemplate(abc.ABC):
    ...
    def verify(self, inst: CronInstance, result: dict) -> dict:
        """Post-action verification (autonomous: post_cc_verify; email: noop).
    ...
    # ----- cookie health gate (BOTH kinds run this before pre_run) -----
    def _cookie_check(self, inst: CronInstance, dry_run: bool) -> Optional[str]:
        ...
        body = (
            ...
            f"修复：在 CC session 跑一次 `python -m src.canvas_login` 重登 Canvas。\n"
            f"下次到点 cron 会自动重试。\n"
        )
    ...
    def run(self, inst: CronInstance, dry_run: bool = False, force: bool = False) -> int:
        ...
        # Tamper detect: authorized_for_course_id should match current course_id
        if inst.authorized_for_course_id is not None and inst.authorized_for_course_id != inst.course_id:
            subject = f"[Canvas cron] Instance tampered — refusing fire ({inst.name})"
            body = (
                f"Instance {inst.name} authorized_for_course_id={inst.authorized_for_course_id!r} "
                f"but current course_id={inst.course_id!r}. Refusing to fire.\n\n"
                f"This means someone hand-edited cron_instances.yaml to change the course "
                f"without re-authorizing through the canvas-cron skill. To fix, delete "
                f"and re-create the instance via skill.\n"
            )
            if not dry_run:
                self.send_email(subject, body, inst.recipient, inst=inst)
            self.log(inst, "TAMPER detected; aborted")
            return 0
        # Cookie health gate (skipped in dry-run)
...
class EmailTemplate(ActionTemplate):
    """Email-only kind. No CC subprocess. Concrete subclasses override
    pre_run / classify / act_dry / act_real."""
    pass


class AutonomousTemplate(ActionTemplate):
    """Autonomous kind. Spawns fresh claude.exe. Provides shared CC-launch,
    post-verify, escalation, and Sunday heartbeat helpers.
    ...
    or hardcoded as class consts):
      - env_var_name: passed to CC subprocess env

    Concrete subclasses MUST override:
      - build_cc_prompt(inst, targets) -> str
      - classify(inst) -> dict including a "targets" list
    """
    CC_TIMEOUT_S = 30 * 60
    EVENTUAL_CONSISTENCY_WAIT_S = 90
    # Resolved at class-attr time: prefer explicit override via env var, then
    # PATH lookup for `claude` / `claude.exe`, else literal "claude" (subprocess
    # will surface a clear FileNotFoundError if it can't be resolved).
    CLAUDE_EXE = Path(
        os.environ.get("CANVAS_CRON_CLAUDE_EXE")
    ...
    def env_var_name(self, inst: CronInstance) -> str:
        """Concrete templates may override; default uses class const."""
        return inst.params.get("env_var_name") or self.DEFAULT_ENV_VAR

    def build_cc_prompt(self, inst: CronInstance, targets: list[dict]) -> str:
        raise NotImplementedError(f"{type(self).__name__} must implement build_cc_prompt()")
    ...
    def fire_cc_session(self, inst: CronInstance, targets: list[dict],
                        dry_run: bool) -> tuple[int, Path]:
        ldir = self.logs_dir(inst)
        ldir.mkdir(parents=True, exist_ok=True)
        ts = dt.datetime.now(PT).strftime("%Y-%m-%d-%H%M%S")
        out_log = ldir / f"cron-{ts}.log"
        prompt = self.build_cc_prompt(inst, targets)
        if dry_run:
            self.log(inst, f"  [dry-run] would fire CC -> {out_log}")
            self.log(inst, f"  [dry-run] prompt preview: {prompt[:300]}...")
            return (0, out_log)
        env = dict(os.environ)
        env[self.env_var_name(inst)] = "1"
        self.log(inst, f"  firing CC, logging to {out_log}")
        with out_log.open("w", encoding="utf-8") as f:
            f.write(f"=== {inst.name} CC session start {ts} ===\n")
            ...
            try:
                result = subprocess.run(
                    [str(self.CLAUDE_EXE), "-p", prompt,
                     "--permission-mode", "bypassPermissions",
                     "--output-format", "text"],
                    cwd=str(ROOT),
                    stdin=subprocess.DEVNULL,
                    stdout=f, stderr=subprocess.STDOUT,
                    timeout=self.CC_TIMEOUT_S,
                    env=env,
                )
                f.write(f"\n=== exit {result.returncode} ===\n")
                return (result.returncode, out_log)
            except subprocess.TimeoutExpired:
                f.write(f"\n=== TIMEOUT after {self.CC_TIMEOUT_S}s ===\n")
                self.log(inst, f"  CC timed out after {self.CC_TIMEOUT_S}s")
                return (124, out_log)
            except Exception as e:
                f.write(f"\n=== EXCEPTION: {e} ===\n")
                self.log(inst, f"  CC subprocess exception: {e}")
                return (1, out_log)

    def post_cc_verify(self, inst: CronInstance, targets: list[dict],
        ...
        for t in targets:
            try:
                ...
                self.log(inst, f"  post-CC verify({t.get('id')}) failed: {e}")
                out[t["id"]] = "error"
        return out

    def email_failure(self, inst: CronInstance, item: dict, ledger: dict,
                      post_state: str, cc_log: Path, body_extras: str = "") -> None:
