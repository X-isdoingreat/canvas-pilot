...
def _normalize_authorization_receipt(raw_path: str | None) -> str | None:
    """Return a repo-relative private receipt path, or reject unsafe storage."""
    if not raw_path:
        return None
    path = Path(raw_path)
    ...
    try:
        ...
        raise ValueError(
            "durable cron authorization receipts must live under _private/"
        ) from exc
    if not resolved.is_file():
        raise ValueError(f"authorization receipt not found: {resolved}")
    return resolved.relative_to(ROOT).as_posix()


def _validate_template_authorization(template, inst: CronInstance) -> bool:
    if template.spec.kind != "autonomous":
        return True
    try:
        template.validate_runtime_authorization(inst)
    except Exception as exc:
        print(
            f"❌ autonomous authorization denied: {type(exc).__name__}: {exc}",
            file=sys.stderr,
        )
        return False
    return True
...
def cmd_create(args) -> int:
    ...
    spec = template.spec
    try:
        receipt_path = _normalize_authorization_receipt(args.authorization_receipt)
    except ValueError as exc:
        print(f"❌ {exc}", file=sys.stderr)
        return 1
    if spec.kind == "autonomous" and receipt_path is None:
        print(
            f"❌ template '{args.template}' is autonomous and requires "
            "--authorization-receipt pointing to a signed durable receipt.",
            file=sys.stderr,
        )
        return 1
    if spec.kind != "autonomous" and receipt_path is not None:
        print(
            "❌ authorization receipts are accepted only by autonomous templates.",
            file=sys.stderr,
        )
        return 1
    # Parse + coerce params
    ...
    inst = CronInstance(
        ...
        created_at=now_iso,
        authorization_receipt=receipt_path,
    )
    if not _validate_template_authorization(template, inst):
        return 1
...
def cmd_fire(args) -> int:
    ...
    if not template:
        print(f"❌ template {inst.template!r} not found", file=sys.stderr)
        return 1
    if args.dry_run and args.os_trigger:
        print(
            "❌ --dry-run and --os-trigger are mutually exclusive; no task was triggered.",
            file=sys.stderr,
        )
        return 1
    if args.os_trigger:
        ...
        if last_result in ("0", "0x0", "267009", "0x41301"):
            ...
            if last_result in ("267009", "0x41301"):
                print(f"   Note: task still running; Codex session continues in "
...
def cmd_status(args) -> int:
    ...
    for name, inst in targets.items():
        ...
        if inst.authorization_receipt:
            print("  authorization: signed durable receipt configured")
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_create.add_argument(
        ...
        help="signed durable receipt under _private/ (required for autonomous templates)",
    )
