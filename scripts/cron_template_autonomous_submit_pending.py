...
unsubmitted assignments and spawns a fresh Claude Code session that runs
...
PROJECT_PATTERN) — it lists ALL unsubmitted items, hands them to the CC
...
dispatches missed work to a fresh CC session that doesn't carry stale
session state.
"""
...
SPEC = TemplateSpec(
    ...
    kind="autonomous",
    description="Spawn fresh CC session and auto-submit pending assignments for a course.",
    param_schema=[
        ParamSpec(
            key="max_items_per_run", type="int", default=2,
            prompt="At most how many assignments per cron fire?",
            description="Cap on how many pending items the CC session will be "
...
class AutonomousSubmitPending(AutonomousTemplate):
    ...
    def build_cc_prompt(self, inst: CronInstance, targets: list[dict]) -> str:
        ...
        return f"""[CANVAS_CRON_AUTO_RUN] Headless automated run, no human present. {self.env_var_name(inst)}=1 set in env.
...
STEP 3 — Invoke /canvas-execute via the Skill tool with the approval spec "做 <comma-separated-indices>" (Chinese form; canvas-execute §3 recognizes it). Example: if matching indices are 3 and 7, say "做 3, 7". Everything else gets user_decision = "defer".
...
- If /canvas-scan produces 0 pending items matching the targets above (e.g. they got submitted between cron-launch and CC-start), exit cleanly — write a note to runs/<today>/REPORT.md and stop.
...
"""
    ...
    def act_real(self, inst: CronInstance, classification: dict) -> dict:
        ...
        exit_code, cc_log = self.fire_cc_session(inst, live_targets, dry_run=False)
        self.log(inst, f"  CC exited {exit_code}, log {cc_log}")
        return {
            "targets": live_targets, "cc_log": cc_log,
    ...
    def verify(self, inst: CronInstance, result: dict) -> dict:
        ...
        if not targets:
            self.save_ledger_atomic(inst, ledger)
            return {}
        cc_log = result.get("cc_log") or Path("")
        ...
        def get_state(t: dict) -> str:
            ...
            try:
                ...
                self.log(inst, f"  post-CC get_submission({t['id']}) failed: {e}")
                return "error"

        states = self.post_cc_verify(inst, targets, get_state)
        for t in targets:
            ...
            if state in ("submitted", "graded"):
                ...
                self.log(inst, f"  ✗ {t['name']}: {state}")
                self.email_failure(inst, t, ledger, post_state=state, cc_log=cc_log)
        self.save_ledger_atomic(inst, ledger)
        return states
