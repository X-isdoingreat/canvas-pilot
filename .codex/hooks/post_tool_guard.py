...
def runner_script_issue(path: Path) -> str | None:
    ...
    if name == "run.py" or name.startswith("run_") and path.suffix.lower() in {".py", ".sh", ".ps1", ".bat"}:
        return f"runner script blocked under runs/: {rel}"
    if name.startswith("_") and path.suffix.lower() in {".py", ".sh", ".ps1", ".bat"}:
        return f"runner script blocked under runs/: {rel}"
    return None
...
def main() -> None:
    ...
    for path in referenced_result_paths(event):
        ...
        ok, err = validate_result_schema(content)
        if not ok:
